...
var bVecXList2 = bVecXList.map((v) =>  v/1 )
var bVecYList2 = bVecYList.map((v) => v/1 )

var bVecZList2 = bVecZList.map((v) => v/1 )
...
papaya.viewer.Viewer = papaya.viewer.Viewer || function (container, width, height, params) {
    ...
    // this.listenerKeyDown2 = papaya.utilities.ObjectUtils.bind(this, this.keyDownEvent2);
...
// change Timepoint events 
papaya.viewer.Viewer.prototype.changeTimePoint = function (value) {
    var timepoint = value/1
    debugger
    // warning of over the limit
    if(timepoint > 80 || timepoint < 0){
        alert('Out of range! Please enter from 0 to 80')
        $('#input-volume-ele').val(timepoint > 80 ? 80 : 0)
    }
    this.currentScreenVolume.gotoTimepoint(timepoint)
    ...
    // set the background color to white ,first params is color, second is intensity modified
    renderer.setClearColor('white', 1)
    var devicePixelRatio = window.devicePixelRatio;
    renderer.setPixelRatio (devicePixelRatio)
    // add the canvas to div circle-container modified
    $('#circle-container').html(renderer.domElement)
    camera.position.z = 5;
    // draw axes modified
    ...
    function drawCircle (radius, color, x, y, z, opacity, transparent) {
        var geometry = new THREE.SphereGeometry(radius,20, 20);
        // set the circle's material modified
        var material = new THREE.MeshLambertMaterial({
            color, // circle's color
            opacity, // circle's opacity
            transparent: transparent // circle's transparent
        });
        var mesh = new THREE.Mesh(geometry,material);
        mesh.position.set(x, y, z);
        scene.add(mesh);
    }
    // make the webgl can rotate modified
    ...
    drawCircle(1, 'dimgrey', 0, 0, 0, 0.15, true) // draw ball inner fyf
    drawCircle(2, 'grey', 0, 0, 0, 0.1, true) // draw ball middle fyf
    drawCircle(3, 'lightgrey', 0, 0, 0, 0.05, true) // draw ball outerside fyf
    // get the  currentTimepoint modified
    const currentIndex = this.currentScreenVolume.currentTimepoint
    const bvalMap = {
        0: {
            r: 1,
            color: 'lightcoral'
    ...
    const rate = bvalMap[bvalList[currentIndex]].r
    // draw every circle with vectorList modified
    for(var i = 0; i < 81; i++ ){
        const everyRate = bvalMap[bvalList[i]].r
        if(currentIndex === i){
            // draw current circle
            drawCircle(0.08, 'black', bVecXList2[currentIndex] * rate, bVecYList2[currentIndex] * rate, bVecZList2[currentIndex] * rate, 1, false) 
        }else{
            // draw all circle when bval is 0, 700, 2000
            drawCircle(0.05, bvalMap[bvalList[i]].color, bVecXList2[i] * everyRate, bVecYList2[i] * everyRate, bVecZList2[i] * everyRate, 1, false) 
            if(bvalList[i] == 0 && i < 80){
                // draw inner circle when bval is 0
                drawCircle(0.05, bvalMap[bvalList[i]].color, bVecXList2[i + 1] * 0.5, bVecYList2[i + 1] * 0.5, bVecZList2[i + 1] * 0.5, 1) 
            }
        }
    }

    // define the light modified
    var light = new THREE.AmbientLight(0xFFFFFF, 1, 500);
    // set the light to 0 0 0,
    light.position.set(0, 0, 0);
    scene.add(light);
    // render the webgl fyf
    function render(){
    ...
    // control the view when mouse change modified
...
papaya.viewer.Viewer.prototype.initializeViewer = function () {
    ...
    if (this.volume.hasError()) {
        ...
        // set this to that modified
        window.thatThis = this
        // add event when input-volume-ele blur by jquery modified
        $('#input-volume-ele').bind('keyup change click', function() {
            debugger
            if (!$(this).data("previousValue") || $(this).data("previousValue") != $(this).val()){
                console.log("changed");           
                $(this).data("previousValue", $(this).val());
                window.thatThis.changeTimePoint($(this).val())
                debugger
                　　
            }
        })
        ...
        var option = {
            tooltip: {
                ...
                formatter: function(param) { 
                    ...
                    return '<div style=" "> '+ param[0].dataIndex + "<br>" + param[0].value + "<br>"+ '</div>'
                }
            },
            xAxis: {
                // type: 'category',
        ...
        window.myChart.on('click', function (params) {
            console.log(params)
            // modified important
            window.thatThis.changeTimePoint(params.dataIndex)
            // window.thatThis.changeTimePoint($(this).val())
            $('#input-volume-ele').val(params.dataIndex)
            $('#input-volume-ele').trigger('change')
            // let data = {
            //   x: params.name,
            //   y: params.value
            // }
        ...
        console.log(this.bgColor, 'bgColorbgColor')
...
papaya.viewer.Viewer.prototype.drawViewer = function (force, skipUpdate) {
    ...
    if (this.container.display) {
        ...
        // add contents before showing modified
    }
...
papaya.viewer.Viewer.prototype.drawViewerToDrawChart = function (force, skipUpdate) {
    ...
    if (this.container.display) {
        // to get the time series modified 
        this.container.display.drawDisplayToDrawChart(this.currentCoord.x, this.currentCoord.y, this.currentCoord.z);
    }
};
...
// papaya.viewer.Viewer.prototype.keyDownEvent2 = function (ke) {
//     // draw cart when keydown modified
//     var xVal = $('#input-voxel-x').val()
//     var yVal = $('#input-voxel-y').val()
//     var zVal = $('#input-voxel-z').val()
// }
...
// 
papaya.viewer.Viewer.prototype.gotoCoordinateToDrawChart = function (coor, nosync) {
    ...
    this.drawViewerToDrawChart(true);
    this.updateSliceSliderControl();
    ...
    this.container.coordinateChanged(this);
    this.drawViewerToDrawChart(false);
};
...
// get time series modified
papaya.viewer.Viewer.prototype.getCurrentValueAtToDrawChart = function (ctrX, ctrY, ctrZ) {
    /*jslint bitwise: true */

    var interpolation = true
    // store the time series modified
    var list = []
    for(var i = 0; i< 81; i++) {
        // get the time series value when x、y、z is certain, and get a time series by change the volume modified
        list.push(this.volume.getVoxelAtCoordinate(
            (ctrX - this.volume.header.origin.x) * this.volume.header.voxelDimensions.xSize,
            (this.volume.header.origin.y - ctrY) * this.volume.header.voxelDimensions.ySize,
            (this.volume.header.origin.z - ctrZ) * this.volume.header.voxelDimensions.zSize,
            i, !interpolation))
    }
    // set the time series to global variable modified
    window.currentTimeSeries = list
};
...
papaya.viewer.Viewer.prototype.getAxial = function (value) {
    // console.log(JSON.stringify(this))
    // x
    // var maxX = this.volume.header.imageDimensions.xDim;
    this.currentCoord.x = value.x
    // if (this.currentCoord.x >= maxX) {
    //     this.currentCoord.x = maxX - 1;
    // }
    // y
    // var maxY = this.volume.header.imageDimensions.yDim;
    this.currentCoord.y = value.y
    // if (this.currentCoord.y >= maxY) {
    //     this.currentCoord.y = maxY - 1;
    // }
    // z
    // var maxZ = this.volume.header.imageDimensions.zDim;
    this.currentCoord.z = value.z
    // if (this.currentCoord.z >= maxZ) {
    //     this.currentCoord.z = maxZ - 1;
    // }
    var list = []
    for(var i = 0; i < 81; i++) {
        list.push(this.atlas.volume.getVoxelAtCoordinate(
            (this.currentCoord.x - this.atlas.volume.header.origin.x) * this.atlas.volume.header.voxelDimensions.xSize,
            (this.atlas.volume.header.origin.y - this.currentCoord.y) * this.atlas.volume.header.voxelDimensions.ySize,
            (this.atlas.volume.header.origin.z - this.currentCoord.z) * this.atlas.volume.header.voxelDimensions.zSize,
            i,  true))
        // list.push(this.currentCoord.x, this.currentCoord.y, this.currentCoord.z, i, false)
    }
    return list
...
// show modal withe charts
papaya.viewer.Viewer.prototype.showModal = function () {
    // debugger
