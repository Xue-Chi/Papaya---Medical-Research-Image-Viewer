...
papaya.ui.Menu.prototype.buildMenuButton = function () {
    ...
    window.isBlock = true
    // add event to show the chart2 modified
    if (this.icons && this.buttonId === 'SPACEButton') {
        $('#SPACEButton').addClass('space-button')
        $('#SPACEButton').click(() => {
            window.isBlock = !window.isBlock
            $('#chart2').css({
                visibility: window.isBlock ? 'hidden':'visible',
                zIndex: window.isBlock ? 0: 1000000
            })
        })
        // add event to hide the chart2 modified when not click space-button element
        // $(document).on('click', ':not(.space-button)', (e) => {
        //     console.log($('#chart2'), '++++')
        //     if(e.target.className === 'papaya-menu-unselectable' && !window.isBlock){
        //         return
        //     }
        //     if(e.target?.dataset?.zrDomId === "zr_0"){
        //         return
        //     }
