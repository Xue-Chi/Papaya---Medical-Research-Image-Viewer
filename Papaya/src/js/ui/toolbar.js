...
papaya.ui.Toolbar.MENU_DATA = {
    "menus": [
        ...
        // add showModal button modified
        {"label": "SPACEButton", "icons": [papaya.ui.Toolbar.ICON_COLLAPSE],
         "method": "showModal" },
        ...
        // {"label": "SPACEButton", "icons": [papaya.ui.Toolbar.ICON_COLLAPSE],
        // "items": [], "method": "showModal", "menuOnHover": false },
    ]
};
...
papaya.ui.Toolbar.prototype.doUpdateImageButtons = function () {
    var ctr, screenVol, dataUrl, data, data1;
    ...
    if (this.container.showImageButtons) {
        ...
        if (ctr === 0) {
            if (screenVol.rgb) {
                data1.menus[0].items = papaya.ui.Toolbar.RGB_IMAGE_MENU_DATA.items;
            } else if (screenVol.dti) {
                data1.menus[0].items = papaya.ui.Toolbar.DTI_IMAGE_MENU_DATA.items;
            } else {
                data1.menus[0].items = papaya.ui.Toolbar.BASE_IMAGE_MENU_DATA.items;
            }
        } else {
            if (screenVol.dti) {
                data1.menus[0].items = papaya.ui.Toolbar.DTI_IMAGE_MENU_DATA.items;
            } else {
                data1.menus[0].items = papaya.ui.Toolbar.OVERLAY_IMAGE_MENU_DATA.items;
            }
        }
            ...
            if (!this.container.combineParametric || !screenVol.parametric) {
                ...
                this.imageMenus.push((this.buildMenu(data1.menus[0], null, screenVol, ctr.toString())));
            }
        }
    }
};
