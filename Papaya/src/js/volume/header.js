...
papaya.volume.Header.prototype.readHeaderData = function (filename, data, progressMeter, dialogHandler,
    ...
    if (nifti.isCompressed(data[0])) {
        let newData = nifti.decompress(data[0]);
    }
    if (nifti.isNIFTI(data[0])) {
        niftiHeader = nifti.readHeader(data[0]);
        niftiImage = nifti.readImage(niftiHeader, data[0]);
        
        if (nifti.hasExtension(niftiHeader)) {
            niftiExt = nifti.readExtensionData(niftiHeader, data[0]);
        }
        let niftiObj = nifti.readHeader(data[0])
        let niString = niftiObj.toFormattedString()
        let fomatNi = niftiObj.getQformMat()
    }
