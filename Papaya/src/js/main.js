...
papaya.Container.prototype.resizeViewerComponents = function (resize) {
    ...
    this.viewerHtml.css({width: dims[0] + 200 + "px"});
    this.viewerHtml.css({height: dims[1] + "px"});
    this.viewerHtml.css({paddingLeft: padding + "px"});
    this.viewerHtml.css({display: 'flex'});
    this.viewerHtml.css({flexDirection: 'row'});
    ...
    this.displayHtml.css({display: 'flex'});
    this.displayHtml.css({flexDirection: 'row'});
    this.display.canvas.width = dims[0];
    // set the bottom display canvas height to zero modified
    this.display.canvas.height = 0;
...
papaya.Container.prototype.buildDisplay = function () {
    ...
    // get bottomContainer element
    this.bottomHtml = $("." + PAPAYA_BOTTOM_CONTAINER_CSS);
    dims = this.getViewerDimensions();
    this.display = new papaya.viewer.Display(this, dims[0]);
    this.displayHtml.append($(this.display.canvas));
    // create volume input number Element modified
    // add input number style modified
    // add min and max modified
    this.displayHtml.append(`<div style="position: relative; top: -700px">
    <div style="margin-left: 10px;width: 400px; height: 400px;z-index: 100000" id="circle-container">
    </div>
    </div>`)
    this.displayHtml.append(`<div style="position: relative; top: -700px;right: 400px;">
    <div style="margin-left: 10px;width: 40px; height: 40px;z-index: 100000;backgroundColor: red" id="des-container">
    <span class="axis-des-container">x-axis:  <span class="color-x"></span></span></br>
    <span class="axis-des-container">y-axis:  <span class="color-y"></span></span></br>
    <span class="axis-des-container">z-axis:  <span class="color-z"></span></span>
    </div>
    </div>`)
    this.displayHtml.append(`<div class="right-container"><div class='value-container' style="display:flex;flex-direction: column;">
    <div><span class='left-label'>x:</span><span class="x-val"></span></div>
    <div><span class='left-label'>y:</span><span class="y-val"></span></div>
    <div><span class='left-label'>z:</span><span class="z-val"></span></div>
    <div><span class='left-label'>bval:</span><span class="bval-val"></span></div>
    <div><span class='left-label'>bvec:</span><span class="bvec-val left-value-span"></span></div>
    <div><span class='left-label'>intensity:</span><span class="voxel-val left-value-span"></span></div>
    <div><span class='left-label'>location:</span><span class="location1-val left-value-span"></span></div>
    </div>
    <div id='input-volume'>
    <div class='input-container'><span>volume:</span><input type='number' min='0' max='80' id='input-volume-ele' />
    </div></div>
    </div>
    `)
    // this.displayHtml.append("<div id='input-volume'><div class='input-container'><span>volume:</span><input type='number' min='0' max='80' id='input-volume-ele' /></div></div>")
    // add echartsContainer
    // this.bottomHtml.append("<div id='chart'></div>")
};
